// here we uppercase the API_KEY to signal it’s something that doesn’t change
const API_KEY = 'p2ZiUHjtY2nMVqWKkKm1FgeEW3CI6IlS';
...
const createVideo = src => {
  ...
  video.autoplay = true;
  video.loop = true;
  video.className = 'video'; //this is how we can set class name using js
  //console.log(video);
...
const searchGiphy = searchTerm => {
  ...
  fetch(
    ...
    // 'https://api.giphy.com/v1/gifs/search?api_key=p2ZiUHjtY2nMVqWKkKm1FgeEW3CI6IlS&q=doggo&limit=50&offset=0&rating=PG-13&lang=en'
    ...
    //here we call our randomChoice function to give us a random result
    ...
    const video = createVideo(src); // function createVideo(src)
...
// here we seperate out our keyup function and
// we can call to it in various places in our code
const doSearch = event =>{
  ...
  // is we only want to run search when we have a search term that is longer than 2 characters
  // and when they press the enter key
  if (event.key === 'Enter' && searchEl.value.length > 2){
    //console.log('search for', searchTerm);
    // here we call to our searchGiphy function and pass along the searchTerm with it
